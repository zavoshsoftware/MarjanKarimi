...
namespace MarjanKarimi.Controllers
{
    public class BlogCommentsController : Controller
    {
        ...
        public ActionResult Edit(BlogComment blogComment)
        {
            if (ModelState.IsValid)
            {
                if (!string.IsNullOrEmpty(blogComment.Response) && blogComment.ResponseDate == null)
                    blogComment.ResponseDate = DateTime.Now;
        ...
        [AllowAnonymous]
        [HttpPost]
        public ActionResult SubmitComment(string name, string email, string body, string id)
        {
            ...
            else
            {
                ...
                Blog blog =
                    db.Blogs.FirstOrDefault(c => c.Id == blogId);
